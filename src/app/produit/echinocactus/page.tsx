...
const product = {
  name: "Echinocactus Grusonii Golden",
  ...
  description: `
    <strong>Echinocactus grusonii</strong>, surnommé "coussin de belle-mère" ou "tonneau doré", est l'un des cactus les plus iconiques et reconnaissables. Cette variété Golden se distingue par ses épines dorées spectaculaires qui brillent au soleil. Parfait comme pièce maîtresse dans un jardin sec ou comme accent dramatique en intérieur, ce cactus sphérique impressionne par sa forme parfaite et sa croissance lente mais régulière.
  `,
  details: [
    "Origine : Mexique (désert de Chihuahua)",
    "Croissance lente mais régulière",
    "Épines dorées spectaculaires",
    "Très résistant à la sécheresse",
    "Forme sphérique parfaite",
    "Idéal pour jardins secs et rocailles",
    "Livraison sécurisée partout en France",
    "Garantie 2 ans"
  ],
  sizes: [
    { id: "small", label: "25-35cm (S)", multiplier: 0.8 },
    { id: "medium", label: "35-45cm (M)", multiplier: 1 },
    { id: "large", label: "45-60cm (L)", multiplier: 1.5 },
    { id: "xlarge", label: "60cm+ (XL)", multiplier: 2.1 }
  ],
  advantages: [
...
export default function EchinocactusProduct() {
  ...
  const shippingCost = useMemo(() => {
    ...
    const size = product.sizes.find(s => s.id === selectedSize);
    switch (size?.id) {
      case "small": return 15;  // Cactus S
      case "medium": return 25; // Cactus M
      case "large": return 35;  // Cactus L  
      case "xlarge": return 45; // Cactus XL
      default: return 25;
    }
