...
const product = {
  name: "Opuntia Ficus-Indica Giant",
  ...
  description: `
    <strong>Opuntia ficus-indica</strong>, communément appelé figuier de Barbarie, est un cactus robuste et spectaculaire originaire du Mexique. Cette variété géante se distingue par ses larges raquettes plates et sa croissance rapide. Parfait pour créer un jardin méditerranéen authentique ou apporter une touche exotique à votre intérieur.
  `,
  details: [
    "Origine : Mexique",
    "Croissance rapide et robuste",
    "Résistant au froid jusqu'à -8°C",
    "Peut produire des fruits comestibles (figues de barbarie)",
    "Livraison sécurisée partout en France",
    "Garantie 2 ans"
  ],
  sizes: [
    { id: "medium", label: "0.8m - 1.2m (M)", multiplier: 1 },
    { id: "large", label: "1.2m - 1.5m (L)", multiplier: 1.4 },
    { id: "xlarge", label: "1.5m - 1.8m (XL)", multiplier: 1.8 },
    { id: "custom", label: "Sur mesure (nous contacter)", multiplier: 2.5 }
  ],
  advantages: [
...
export default function OpuntiaProduct() {
  ...
  const shippingCost = useMemo(() => {
    ...
    const size = product.sizes.find(s => s.id === selectedSize);
    switch (size?.id) {
      case "medium": return 20; // Cactus M
      case "large": return 30;  // Cactus L  
      case "xlarge": return 40; // Cactus XL
      case "custom": return 60; // Sur mesure
      default: return 30;
    }
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      ...
      quantity: quantity,
      unitPrice: Math.round(totalPrice / quantity), // Prix unitaire sans remise
